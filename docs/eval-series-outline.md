...
- 评测项目是**两个仓库 × 三段流程 × 两套指标**——
  - 仓库：`ragent`（被测）+ `bitselect-rag-assistant`（评测）
...
- 两个仓库的职责切分：`ragent` 只暴露被评接口，`bitselect-rag-assistant` 负责评测集和指标
- 两套指标互补关系图：
...
**正文骨架**：
- 三步初始化（对应 `eval/init/`）：
  - `create_kbs.py`：建 4 个知识库（按文档类型切：PROD / GUIDE / MANUAL+CODE / FAQ）
...
### 第 10 篇：Judge 同源偏置、方差、中文 NaN：RAGAS 的三个坑
...
| 4 | 数据有了，开始算指标，先讲意图和检索 | 自建指标讲完意图和检索，剩下性能 |
| 5 | 意图错则全错，检索是结果落地的前提 | 性能口径要细抠 |
